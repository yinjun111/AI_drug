#!/usr/bin/env python3
"""
Generate a comprehensive drug-disease-category table for all drugs in purplebook_merged_proper_name.csv
One drug-disease association per row.
"""
import csv
import json
import re
...
OUTPUT_CSV = Path("purplebook_adalimumab_style_drug_diseases.csv")
CACHE_FILE = Path("fda_indications_cache_full.json")
...
def main():
    ...
    for idx, (drug_name, proprietary_name) in enumerate(drugs, start=1):
        ...
        gene_display = drug_name
        brand_display = proprietary_name if proprietary_name else ""
        print(f"[{idx}/{len(drugs)}] Querying Gene='{gene_display}' Brand='{brand_display}'")

        # Try to find results from cache or query
        results = []
        chosen_query = None
        ...
        if not results:
            print(f"  No FDA records found for '{drug_name}' (Gene='{gene_display}')")
            rows.append({"Drug Name": drug_name, "Disease": "", "Category": "others"})
            continue

        first_result = results[0] if results else {}
        openfda = first_result.get("openfda", {}) or {}
        generic_name = openfda.get("generic_name")
        brand_name = openfda.get("brand_name")
        generic_name = generic_name[0] if isinstance(generic_name, list) and generic_name else generic_name
        brand_name = brand_name[0] if isinstance(brand_name, list) and brand_name else brand_name
        print(f"  Matched query='{chosen_query}' generic='{generic_name}' brand='{brand_name}'")

        # Extract diseases from all results
        ...
        if not unique_diseases:
            print(f"  Found indications for '{drug_name}' but could not parse diseases (Gene='{gene_display}')")
            rows.append({"Drug Name": drug_name, "Disease": "", "Category": "others"})
            continue

        for disease, category in unique_diseases.items():
            rows.append({"Drug Name": drug_name, "Disease": disease, "Category": category})

        print(f"  Found {len(unique_diseases)} diseases for '{drug_name}' (Gene='{gene_display}')")

    print(f"\nWriting {len(rows)} rows to {OUTPUT_CSV}")
    with OUTPUT_CSV.open("w", encoding="utf-8", newline="") as handle:
        writer = csv.DictWriter(handle, fieldnames=["Drug Name", "Disease", "Category"])
        writer.writeheader()
