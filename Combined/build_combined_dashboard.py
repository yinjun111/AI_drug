...
# ── FIG 5 — Sunburst: Category → Gene → Drug ─────────────────────────────────
def build_sunburst():
    d = df[df["Disease / Indication"] != ""].copy()
    d["g1"] = d["Drug Target (Gene)"].apply(lambda t: (str(t).split(",")[0].strip() or "Other"))
    d["drug_s"] = d["Drug"].apply(lambda x: x[:26] + "…" if len(str(x)) > 28 else x)
    d = d.drop_duplicates(subset=["Disease Category", "g1", "Drug"])
    ids, labels, parents, vals = ["root"], ["All Drugs"], [""], [0]
    for cat, g in d.groupby("Disease Category"):
        cid = "c|" + cat; ids.append(cid); labels.append(cat); parents.append("root"); vals.append(len(g))
        for gene, g2 in g.groupby("g1"):
            gid = cid + "|" + gene; ids.append(gid); labels.append(gene); parents.append(cid); vals.append(len(g2))
            for drug, g3 in g2.groupby("drug_s"):
                ids.append(gid + "|" + drug); labels.append(drug); parents.append(gid); vals.append(len(g3))
    ...
    fig.update_layout(title=dict(text="<b>Disease Category → Target Gene → Drug</b><br><sup>click to drill down</sup>", font=dict(size=14)),
                      margin=dict(l=10, r=10, t=55, b=10), height=620, paper_bgcolor="#F8FAFC")
    return fig
...
<div class="sec s4">Step 4 — Drill-down: Disease → Target → Drug &amp; Gene × Category</div>
<div class="g2"><div class="card">{to_div(f_sun,"sun")}</div><div class="card">{to_div(f_heat,"heat")}</div></div>
